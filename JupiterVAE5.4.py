...
# ## VAE 5.4
# 
# gets rid of last batchnorms,
# Tanh instead of Leaky ReLU in hidden
# 
...
vortices = '/home/rsankar/zooniverse/projects/junocam/junodata/vortex_data.nc'
...
training_vortices, val_vortices = create_generators(vortices, batch_size = batch_size)
...
class Encoder(nn.Module): 
    def __init__(self, conv_filt, hidden, input_channels=3): #conv_filt
        ...
        for i in range(nconv):
            ...
            self.layers.append(nn.Tanh()) #Tanh?
...
vae.load_state_dict(torch.load('vae5.4.pth'))
...
with torch.no_grad():
        ...
        fig = plt.figure(figsize=(16, 16))
        batch = training_vortices[0].to(device) #training_data[0] is the first batch of images
        val_batch = val_vortices[0].to(device)
...
for sample_img in tqdm.tqdm(training_vortices): 
    img = sample_img.to(device)
    label = sample_img[1]
    vae.eval() #encode image
...
reducer1 = umap.UMAP(random_state=1, min_dist=0, n_components=3) #n_components = 3
embedding1 = reducer1.fit_transform(encoded_samples)
embedding1.shape

reducer2 = umap.UMAP(random_state=1, min_dist=0)
embedding2 = reducer2.fit_transform(encoded_samples)
embedding2.shape
...
plt.scatter(
    embedding2[:, 0],
    embedding2[:, 1], s=1, color='cornflowerblue', alpha=.25
...
fig, axs = plt.subplots(1, 2, dpi=200, sharey=True, figsize=(12,5))



axs[0].scatter(embedding1[:,0], embedding1[:,1], 
                  s=0.2, color='cornflowerblue', alpha=.25)
axs[1].scatter(embedding1[:,2], embedding1[:,1], 
                  s=0.2, color='cornflowerblue', alpha=.25)

axs[0].set_xlabel(r'UMAP0')
axs[0].set_ylabel(r'UMAP1')
axs[1].set_xlabel(r'UMAP2')
...
batch_indices = training_data.indices[mask_indices] #np.where(clusterer.labels_==label)[0] for a given cluster
...
clusterer = hdbscan.HDBSCAN(min_cluster_size=20, min_samples=15, cluster_selection_method='eom').fit(embedding2)
#clusterer = OPTICS(xi=0.002, max_eps=6, metric='sqeuclidean', min_samples=40).fit(embedding2)
...
plt.scatter(embedding2[non_invalid,0], embedding2[non_invalid,1], s=0.1, c=clusterer.labels_[non_invalid], cmap='gist_rainbow')
plt.colorbar()
...
plt.figure(figsize=(12,9))
plt.scatter(embedding2[non_invalid,0], embedding2[non_invalid,1], 
...
#clusterer = hdbscan.HDBSCAN(min_cluster_size=50, min_samples=50,cluster_selection_method='leaf').fit(embedding1)
clusterer = OPTICS(xi=0.002, max_eps=4, metric='sqeuclidean', min_samples=30).fit(embedding1)

fig, axs = plt.subplots(1, 2, dpi=150, sharey=True, figsize=(8,5))

non_invalid = clusterer.labels_ > -1

print(sum(non_invalid)/len(clusterer.labels_))

axs[0].scatter(embedding1[non_invalid,0], embedding1[non_invalid,1], s=0.1, c=clusterer.labels_[non_invalid], cmap='gist_rainbow')
axs[1].scatter(embedding1[non_invalid,2], embedding1[non_invalid,1], s=0.1, c=clusterer.labels_[non_invalid], cmap='gist_rainbow')
...
# cluster_indices=np.where(clusterer.labels_==3)[0] 

# batch_indices = training_data.indices[cluster_indices] #np.where(clusterer.labels_==label)[0] for a given cluster
...
def plot_cluster_images(label):
    ...
    cluster_indices=np.where(clusterer.labels_==label)[0]#[401:500] 

    batch_indices = training_vortices.indices[cluster_indices] #np.where(clusterer.labels_==label)[0] for a given cluster

    with nc.Dataset(training_vortices.nc_file, 'r') as dset:
        imgs = dset.variables['imgs'][batch_indices,:,:,:]
    ...
    with torch.no_grad():
        imgs= np.transpose(imgs, (0,3,1,2))    
        img_subset = torch.Tensor(imgs).to(device)
        img_decoded = vae(img_subset).cpu().numpy()
        img_decoded = np.transpose(img_decoded, (0,2,3,1))
    ...
    #     i += 30
        ax1.imshow(img_decoded[i], cmap='gray')
    ...
    cont = ax.scatter(embedding2[non_invalid,0], embedding2[non_invalid,1], 
                      s=0.1, color='gray')

    cont = ax.scatter(embedding2[cluster_indices,0], embedding2[cluster_indices,1], 
                      s=0.1, c=clusterer.labels_[cluster_indices], cmap='gist_rainbow', vmin=0, vmax=12)
    ax.set_xticks([])
    ax.set_yticks([])
    plt.tight_layout()
    plt.show()

def plot_original_images(label):

    cluster_indices=np.where(clusterer.labels_==label)[0]#[200:300] 
...
plot_original_images(24)


# In[82]:


plot_cluster_images(24)
